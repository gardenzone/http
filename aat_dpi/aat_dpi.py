...
def read_dpi_config(file_path):
    ...
    for row_num in range(3, row_count):
        ...
        # 在原始 excel 的 备注数据2 列，添加该行数据使用的修改模板
...
def verify_case_pass(log, lines):
    # fail: 验证失败, pass: 验证通过
    # case 的 log 自上往下执行，如果因为某个步骤 fail 了导致后面的 excel 数据没有执行，
    # 则该步骤标记为 fail,后面的数据下个 case 重新执行
    result = []
    epg_logs = []
...
if __name__== "__main__" :
    ...
    try:
        ...
        type_date = {'l3+l7+http': [{'line_num': 918, 'flag': '新增', 'description': '新增组合规则', 'code': 1750000006, 'name': 'mgsp_00', 'l3': '111.40.195.239', 'protocol': '', 'l4': '', 'l7': 'http://*mgsplive.miguvideo.com/*'}, {'line_num': 3182, 'flag': '新增', 'description': '新增组合规则', 'code': 1750000089, 'name': 'xmly_00', 'l3': '111.13.40.91', 'protocol': '', 'l4': None, 'l7': 'http://*.xmcdn.com/*'}], 'l3': [{'line_num': 3179, 'flag': '新增', 'description': '新增三层规则', 'code': 1740000002, 'name': 'mgspqwdx_00', 'l3': '39.173.75.14', 'protocol': None, 'l4': None, 'l7': None}], 'l7+https': [{'line_num': 3181, 'flag': '新增', 'description': '新增七层规则', 'code': 1740000002, 'name': 'mgspqwdx_00', 'l3': None, 'protocol': None, 'l4': None, 'l7': 'https://*.aikan.miguvideo.com'}], 'l3+l4+l7+https': [{'line_num': 3258, 'flag': '新增', 'description': '新增组合规则', 'code': 1750000089, 'name': 'xmly_00', 'l3': '111.13.40.91', 'protocol': '', 'l4': 443, 'l7': 'https://*.xmcdn.com'}]}
